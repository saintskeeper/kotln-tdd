...
data class HttpInterpreter(val env: String = "local") : ZettaiInterpreter {
    ...
    override fun renameList(user: User, oldList: ListName, newList: ListName) {
        val response = submitToZettai(renameListUrl(user, oldList), renameListForm(newList))

        expectThat(response.status).isEqualTo(Status.SEE_OTHER)  //redirect same page
    }

    private fun newListForm(listName: ListName): Form = listOf("listname" to listName.name)
    private fun renameListForm(newList: ListName): Form = listOf("newListName" to newList.name)
