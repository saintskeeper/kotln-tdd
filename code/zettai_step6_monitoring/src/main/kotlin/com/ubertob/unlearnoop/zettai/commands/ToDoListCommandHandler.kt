...
class ToDoListCommandHandler<CTX>(
    ...
    private fun CreateToDoList.execute(): CommandOutcomeReader<CTX> =
        eventStore.retrieveByNaturalKey(UserListName(user, name))
            .transform { listState ->
                when (listState) {
                    null -> ListCreated(id, user, name).toList().asSuccess()
                ...
                when (listState) {
                    is ActiveToDoList -> {
                        if (newNameList != null)
                            ...
                            ListRenamed(listState.id, user, newName).toList().asSuccess()
                    }
    ...
    private fun RenameToDoList.retrieveOldAndNewListName(): ContextReader<CTX, Pair<ToDoListState?, ToDoListState?>> =
        eventStore.retrieveByNaturalKey(UserListName(user, oldName)).bind { currList ->
            eventStore.retrieveByNaturalKey(UserListName(user, newName)).transform { currList to it }
        }
